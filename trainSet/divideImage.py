...
# imageY = 10
# imageNumber = 100
imagePath = 'imagesTrain'
labelPath = 'labelsTrain'
...
#    print(str(imageFile).zfill(5))
    ...
    label = cv2.imread(labelPath+'/'+labelList[imageFile])
    for r in range(0, image.shape[0]-imageSize, imageSize):
        for c in range(0, image.shape[1]-imageSize, imageSize):
            ...
            cv2.imwrite('./labels/ds_'+str(n).zfill(5)+'.png', label[r:r+imageSize, c:c+imageSize, :])
            n = n+1
